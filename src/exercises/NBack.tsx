...
export function NBack({ difficulty, onComplete, timeUp, progressive }: ExerciseProps) {
  const { level, onRoundEnd } = useProgressiveDifficulty(difficulty, progressive);
  const params = getParams(level);

  const [sequence, setSequence] = useState(() => generateSequence(params.n, params.trialsPerRound));
  ...
  currentNRef.current = params.n;

  const isMatch = currentIndex >= currentNRef.current && sequence[currentIndex] === sequence[currentIndex - currentNRef.current];
  ...
  const finalize = useCallback(() => {
    ...
    onComplete({
      ...
      total,
      accuracy: acc,
      score: normalizeScore(totalCorrect, total, level),
      durationMs: Date.now() - startTime,
      rounds: round,
    });
  }, [totalCorrect, totalMatches, round, level, onComplete, startTime]);
  ...
  const endRound = useCallback(() => {
    ...
    setSequence(generateSequence(nextParams.n, nextParams.trialsPerRound));
    setCurrentIndex(0);
    setResponded(false);
    setRoundHits(0);
    setFeedback(null);
    if (passed) setRound((r) => r + 1);
  }, [sequence, roundHits, totalCorrect, totalMatches, timeUp, onRoundEnd, level, progressive]);
  ...
  useEffect(() => {
    if (timeUp && !completedRef.current) finalize();
  }, [timeUp, finalize]);
  ...
  return (
    <div className="flex flex-col items-center gap-6 min-h-[40vh] justify-center">
      ...
      <p className="text-text-muted text-sm">
        Press Match when the letter matches {params.n} step{params.n > 1 ? 's' : ''} back
      </p>

      <div className={`w-32 h-32 rounded-2xl flex items-center justify-center text-6xl font-bold transition-colors ${feedback === 'hit' ? 'bg-accent/20 text-accent' : feedback === 'false-alarm' ? 'bg-danger/20 text-danger' : 'bg-bg-card border-2 border-border text-text'}`}>
        {sequence[currentIndex]}
      </div>

      <button onClick={handleMatch} disabled={responded} className="px-12 py-4 rounded-xl bg-primary text-white font-bold text-xl active:scale-95 transition-transform disabled:opacity-40">
        Match!
      </button>

      <p className="text-text-muted text-xs">
        {currentIndex + 1} / {params.trialsPerRound} · {params.n}-back
      </p>
    </div>
  );
}
